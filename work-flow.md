...
## 2026-09-01｜修复登录页面 HTML 响应报错与注册交互

**【时间】** 2026-09-01 21:34（Asia/Shanghai）
**【事件】** 修复登录页面显示 `Unexpected token '<'`，并完善空表单注册交互。
**【问题来源】** 用户提供的本地与 GitHub Pages 登录截图。
**【执行方向】**
1. 追踪 `/api/auth/*` 请求在 GitHub Pages 上被静态站点返回 HTML 的原因。
2. 认证请求统一读取响应文本，识别 HTML、非 JSON、HTTP 错误并转换为可操作的中文提示。
3. 将“注册账号”改为独立注册弹窗，增加确认密码和不一致校验。
4. 同步根目录、`public/`、`docs/` 三处页面资源。
**【执行边界】**
- 不修改认证后端协议、设备试用锁、管理员权限和排盘算法。
- 不提交管理员凭据、API 密钥和未跟踪临时材料。
**【执行结果】**
- 公网静态响应不再直接进入 `response.json()`，提示改为“公网预览不提供账号服务，请打开本地完整服务”。
- 本地错误账号显示“账号或密码错误”，不再显示 JSON 解析异常。
- 空表单点击“注册账号”打开注册弹窗；确认密码不一致时显示明确校验信息。
**【执行验证】**
- 本地真实页面：注册弹窗可打开，密码校验通过，错误登录提示为“账号或密码错误”。
- 375×812 窄屏截图检查：弹窗完整显示，无横向溢出。
...
**【事件】** 修复 GitHub Pages 公网页面与本地 `8090` 页面不一致的问题。
**【问题来源】** 用户发现公网网址仍显示旧版页面，与本地开发页面严重不一致。
**【执行方向】**
1. 对比本地页面、`master-doc` Raw 文件、`master` Raw 文件和 GitHub Pages 实际响应。
2. 确认 GitHub Pages 当前从 `master` 发布，而最新代码位于 `master-doc`。
3. 经用户明确授权，将 `master-doc` 以快进方式同步到 `master`。
4. 重新验证两个远程分支提交号和公网文件特征。
**【执行边界】**
- 仅进行快进同步，不改写 Git 历史。
- 不提交未跟踪的截图、临时脚本和研究目录。
- 不提交管理员凭据与 API 密钥。
**【执行结果】**
- 根因：Pages 发布源分支与实际更新分支不一致。
- 修复方式：使 `master` 与 `master-doc` 指向同一最新提交。
**【相关文档】** `master`、`master-doc`、`work-flow.md`
...
**【时间】** 2026-09-01（Asia/Shanghai）
**【事件】** 按用户要求准备将当前 `master-doc` 工作区更新推送到 GitHub，并将 GitHub Pages 公网地址写入项目规范。
**【问题来源】** 用户要求“推送并更新 GitHub”“公网网址”“把 GitHub 网址写入 AGENTS”。
**【执行方向】**
1. 保持远程仓库 `https://github.com/142857110823/app-for-father.git`。
2. 使用 `master-doc` 分支提交当前项目变更。
3. 登记 GitHub Pages 地址 `https://142857110823.github.io/app-for-father/`。
4. 不提交 API 密钥、管理员本地凭据、数据库和构建密钥。
**【执行边界】**
- 不回滚用户已有未提交文件。
- 不把 `API密钥.txt`、`admin-credentials.local.json`、`data/`、APK 或本地运行凭据推送到仓库。
**【执行结果】**
- 已在 `AGENTS.md` 的项目概述和公网访问约束中登记仓库地址与 GitHub Pages 地址。
- 当前推送前的工作区提交与远程同步仍待网络权限完成。
**【相关文档】** `AGENTS.md`、`work-flow.md`

---

## 2026-08-31｜首尾宫位加粗与公历农历联动

**【时间】** 2026-08-31（Asia/Shanghai）
**【事件】** 为 `2首/2尾、4首/4尾、6首/6尾、8首/8尾` 八个宫位增加区分边框，并修复公历/农历切换逻辑，删除“四柱”历法入口。
**【问题来源】** 用户提出的排盘界面区分性和出生时间自动修正要求。
**【执行方向】**
1. 给 8 个首尾宫位增加独立 `split-palace` 标识和粗黑边框。
2. 默认使用当前公历时间。
3. 切换农历时将当前公历日期转换为农历并更新出生时间显示。
4. 农历时间选择后转换回公历供排盘算法使用。
5. 删除历法中的“四柱”按钮，同步三处页面和浏览器算法包。
**【执行边界】**
- 不改变十三宫排盘规则、双遁算法、宫位数据和四柱结果字段。
- 农历/公历只负责输入显示与日期转换，核心排盘内部仍使用公历时间。
**【执行结果】**
- 新增 `solarToLunar`、`lunarToSolar` 转换函数并暴露到浏览器算法包。
- 8 个首尾宫位屏幕与导出边框统一为约 2.5px 黑色边框。
- 公历 `2026-08-31 13:46` 可切换为农历 `2026年7月19日 13:46`，并可准确转换回原公历时刻。
- 删除 `cal-pillar` 入口及“四柱”历法映射。
- 已同步 `index.html`、`public/index.html`、`docs/index.html`、`algorithm.bundle.js`。
**【执行验证】**
- `node tests/qimen-core.test.js`：7 项全部通过。
- `node tests/paipan-render.test.js`：27 项全部通过。
- 真实页面交互验证：默认公历显示正常；点击农历后日期自动更新；排盘结果 8 个首尾宫位均检测到四边约 `2.4px` 边框。
- 375×812 窄屏截图检查：边框清晰，页面无重叠、截断或溢出。
- 本地预览地址：`http://localhost:8090/`。
**【相关文档】** `algorithm/pillars.js`、`algorithm/browser-entry.js`、`public/index.html`、`index.html`、`docs/index.html`、`algorithm.bundle.js`、`tests/qimen-core.test.js`、`tests/paipan-render.test.js`、`work-flow.md`

---

## 2026-08-31｜排盘结果黑白化与日局留白

**【时间】** 2026-08-31（Asia/Shanghai）
**【事件】** 修正【排盘结果】中贵神宫淡黄色标注，并调整【二十四节气】与【日局】的垂直间距。
**【问题来源】** 用户提供的排盘截图及两项视觉修正要求。
**【执行方向】**
1. 贵神宫与其他宫位统一使用黑白/白底样式。
2. 二十四节气下方增加留白，日局向宫位下方调整。
3. 同步屏幕渲染、导出样式及根目录、`public/`、`docs/` 三处页面。
**【执行边界】**
- 不修改十三宫排盘算法、宫位字段、双遁逻辑和权威文档规则。
- 附件仅作为视觉参考，不作为算法规则来源。
**【执行结果】**
- `#plate-table td.gui-shen` 改为 `var(--plate-cell)`，移除淡黄色渐变。
- `.pc-jq` 增加 `margin-bottom: 8px`，`.pc-rp` 增加 `margin-top: 8px`；窄屏分别调整为 5px。
- 导出排盘贵神宫同步改为纯白背景。
- 已同步 `index.html`、`public/index.html`、`docs/index.html`。
**【执行验证】**
- `node tests/paipan-render.test.js`：21 项全部通过。
- 桌面端实际案例：贵神宫背景计算值为 `rgb(255, 255, 255)`；节气与日局间距实测 16px，无重叠。
- 375×812 窄屏实际截图检查：宫位、节气、日局均可见，未发现重叠、截断或溢出。
- 审查证据：`artifacts/paipan-monochrome-desktop-20260831.png`、`artifacts/paipan-monochrome-mobile-20260831.png`。
- 审查时间：2026-08-31；本地预览地址：`http://localhost:8090/`。
**【相关文档】** `public/index.html`、`index.html`、`docs/index.html`、`tests/paipan-render.test.js`、`work-flow.md`
...
### 2026-08-31 六合历史开奖研究项目治理初始化

**【时间】** 2026-08-31（Asia/Shanghai）  
**【事件】** 在 `六合` 子目录建立台湾大乐透与澳门白鸽票历史研究的项目规范、项目指南、实时工作流、设计说明、执行计划和三个业务智能体合同。  
**【问题来源】** 用户确认全部按推荐方案执行，并要求在 `F:\1\夫\六合` 配置 `AGENTS.md`、`项目指南.md`、`work-flow.md`，同时规范化三个分工互斥的 subagent。  
**【执行方向】**
1. 台湾智能体负责官方大乐透历史数据。
2. 澳门智能体负责官方白鸽票历史数据。
3. 统计集成智能体负责质量控制、统计研究和两个独立 XLSX。
4. 通过来源登记、目录写入边界和统一交接格式避免数据污染与职责冲突。
**【执行边界】**
- 本次阶段只建立项目治理和执行合同，尚未开始抓取开奖数据。
- 不触碰现有奇门排盘页面、算法、APK、前端资源和无关未提交修改。
- 不采集地下六合彩或来源不明数据，不提供投注号码、投注建议或准确率承诺。
**【执行结果】**
- `六合` 内项目治理文件、三个角色文件、设计说明和执行计划已创建。
- 数据目录按台湾、澳门、分析和输出划分，三名智能体的写入范围互不重叠。
- 计划采用逐任务实现、逐任务审查和全项目终审。
**【相关文档】** `AGENTS.md`、`项目指南.md`、`work-flow.md`、`01-台湾数据采集智能体.md`、`02-澳门数据采集智能体.md`、`03-统计分析与工作簿集成智能体.md`、`2026-08-31-liuhe-history-research-design.md`、`2026-08-31-liuhe-history-research.md`

### 2026-08-31 联系我们二维码再次更新

**【时间】** 2026-08-31（Asia/Shanghai）
**【事件】** 按用户要求，将【我的】→【联系我们】二维码更新为本次提供的图片，并准备同步 GitHub。
**【问题来源】** 用户提供的新二维码图片及自动推送要求。
**【执行方向】**
1. 保持现有页面引用 `二维码.jpg`、文案、布局、预览、保存和转发逻辑不变。
2. 将新图片同步到根目录、`public/`、`docs/` 三处资源。
3. 验证本地资源和页面显示后，仅提交本次二维码资源并推送当前 `master-doc` 分支。
**【执行边界】**
- 不提交工作区中其他任务的未提交修改。
- 不修改排盘算法、业务逻辑和其他页面。
**【执行结果】**
- 新二维码图片已同步至三处 `二维码.jpg`。
- 用户图片 SHA256 为 `5085FCFE6C5FB0D430910D54F0DD3AB38D4BD308BFD5DE4C707915EE05681018`。
**【执行验证】**
- 三处资源 SHA256 均为 `5085FCFE6C5FB0D430910D54F0DD3AB38D4BD308BFD5DE4C707915EE05681018`，图片尺寸均为 `736×1354`。
- 本地资源请求 `http://localhost:8090/二维码.jpg` 返回 HTTP `200`，`Content-Type=image/jpeg`，响应长度 `129237` 字节。
- 本地浏览器验证时间：2026-08-31；进入【我的】后二维码 `src` 仍为 `二维码.jpg`，图片实际加载完成，桌面与 `375×812` 视口均显示为 `72×72`，窄屏 `scrollWidth=375`、`clientWidth=375`，无横向溢出。
- 浏览器截图写入接口未在工作区生成文件，未将不存在的截图作为证据；本次以实际 DOM 尺寸、自然尺寸、加载状态和溢出数据作为视觉核验记录。
