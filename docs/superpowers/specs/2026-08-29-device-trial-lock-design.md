...
> 2026-08-31 修订：按用户最新要求，试用期限从 3 天调整为 1 天；服务端设备会话判定优先于前端存储，VIP 与管理员不受限制。

## 1. 背景与目标

为软件增加一层设备级使用限制：**同一台设备首次登录后仅有 1 天完整使用期**，到期后核心排盘功能被锁定，但允许继续浏览非核心页面。

约束：
- 不修改 URL。
- 1 天内前端无任何弹窗、提示或倒计时。
- 到期后弹出「管理员解锁」和「会员充值」两个入口。
- 管理员账号密码预先准备，不受 1 天限制。

## 2. 核心规则

- 计时起点：**首次打开 App/Web 页面**（页面脚本首次执行）。
- 计时长度：**1 天**（`1 × 24 × 60 × 60 × 1000 ms`）。
- 到期前：不显示任何与试用期相关的 UI。
- 到期后：
  - 禁用「开始排盘」等核心入口。
  - 点击被禁用的入口时弹出锁定窗口。
...
- 锁定弹窗：
  - 标题：「本设备试用已到期」
  - 说明：「每台设备首次登录后可免费使用 1 天，如需继续使用请充值会员或联系管理员。」
...
- 服务端以 `X-Device-Id` 对设备会话进行持久判定；客户端清空浏览器存储不能撤销已记录的设备期限。
- 管理员密码不硬编码在前端，仅通过后端接口校验。
- 不显示倒计时、浮窗、角标，避免 1 天内打扰用户。
